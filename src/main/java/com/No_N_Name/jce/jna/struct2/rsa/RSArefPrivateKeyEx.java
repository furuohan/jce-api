...
public class RSArefPrivateKeyEx extends Structure implements IRSArefPrivateKey{
    public int bits;							//模长
    public byte[] m = new byte[512];			//模N
    public byte[] e = new byte[512];			//公钥指数
    public byte[] d = new byte[512];			//私钥指数
    public byte[] prime = new byte[512];		//素数p和q
    public byte[] pexp = new byte[512];			//Dp和Dq
    public byte[] coef = new byte[256];			//系数i
    ...
    public void decode(byte[] bytes) throws CryptoException {
        ...
        if (pos != bytes.length) {
            throw new CryptoException("inputData length != ExRSAPrivateKey length");
        }
    }
    ...
    public byte[] encode() throws CryptoException {
        ...
        try {
            ...
            throw new CryptoException("RSArefPrivateKeyEx encode error.", var3);
        }
